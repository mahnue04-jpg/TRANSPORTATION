...
APPLICANT_UNAUTHORIZED = "Not authorized."
...
def _payload_matches_driver_001(
    ...
    """Locate a Driver 001 candidate. Matching never grants applicant access."""
    if payload is None:
        return False
    ...
    if first == "driver" and last == "001":
        return True
    if "drv-001" in claimed or "driver 001" in claimed:
        return True
    return False


def _driver_001_candidates(
    db: Session,
    *,
...
def create_draft_application(
    ...
    *,
    ...
    for existing in _driver_001_candidates(db, organization_id=organization_id):
        if _payload_matches_driver_001(payload, existing):
            # Identity can locate Driver 001 but never authorizes read or write access.
            raise ValueError(APPLICANT_UNAUTHORIZED)
