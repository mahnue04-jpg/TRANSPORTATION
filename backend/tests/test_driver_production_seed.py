...
def test_list_drivers_self_heals_from_peer_org_fleet() -> None:
    ...
    with SessionLocal() as db:
        ...
        if peer_org is None:
            peer_org = HealthISFOrganization(
                id=uuid4(),
                name="Peer Fleet Org",
                code="AMICOR-PEER-FLEET",
                address="1 Peer Ave",
                phone="212-555-0199",
                is_active=True,
            )
        ...
        health_isf_service.ensure_sample_providers(db, organization_id=org_id)
        db.query(HealthISFDriver).filter(HealthISFDriver.organization_id == org_id).delete()
        health_isf_service.ensure_sample_drivers(db, organization_id=str(peer_org.id))
        db.commit()
